...
/*  
  Когда мы ссылаемся на свойство объекта (например через точку), то если этого свойства не существует, js использует ссылку на прототип этого объекта,
  чтобы поискать это свойство у прототипа
*/

// @ связывание объекта и прототипа
const foo4 = {
  a: 42
}
// Object.create возвращает объект у которого Prototype будет ссылться на foo4
let bar4 = Object.create(foo4)
bar4.b = "hello world"
console.log(bar4)
console.log(bar4.b)
console.log(bar4.a) // свойство а не будет найдено в bar4 и тогда js будет использовать объект на который указывает ссылка Prototype, тоесть объект foo4

//§  Полифилинг(polyfilling) и транпиляция(transpiling)

/* 
Заполифилить функцию значит - написать свой аналог функции, которая уже есть в последней версии js, используя при этом 
не самый новый синтаксис языка, чтобы можно было пользоваться этой функцие незвисимо от того поддерживает ли старый браузер ее или нет.
То есть в поддерживающих браузерах последнюю верисю языка будет использована стандартная функция, а в неподдерживающих будет использован наш полифил.
*/

//@ Пример как можно заполифилить функцию 
// Прям глобальной области пишем, теперь у браузеры со старым js будут при запуске нашего когда использовать нашу заполифилиную функцию isNaN
if (!Number.isNaN) {
  Number.isNaN = function (x) {
    return x !== x
  }
}
// Необязательно писать полифилы самому, есть уже протестированные наборы полифилов

/* 
Транспиляция - конвертация кода написанного на новом синтаксисе в эквивалент кода в старом синтаксисе. Нужно,чтобы запускать код в старых браузерах.
Транспиляциия, как объединение терминов трансформация и компиляция (transforming + compiling)
Babel, Traceur
*/